...
class BotTester(object):
    ...
    def __init__(self, drafts):
        ...
        self.correct = pd.DataFrame(
            columns=['draft_num', 'pick_num', 'human_pick'], index=range(self.n_packs))
        self.fuzzy_correct = pd.DataFrame(
            columns=['draft_num', 'pick_num', 'human_pick'], index=range(self.n_packs))
        self.rank_error = pd.DataFrame(
            columns=['draft_num', 'pick_num', 'human_pick'], index=range(self.n_packs))
    ...
    def evaluate_bots(self, bots, bot_names):
        ...
        for bot_counter in range(len(bots)):  # AKh: better to rename to iBot
            ...
            for draft_num in range(len(self.drafts)):
                ...
                for pick_num in range(len(draft)):
                    ...
                    fuzzy_correct = self.is_bot_correct(
                        pack, pack_rank, fuzzy=True)
        ...
        # All card names; human_pick is just where they came from
        self.card_acc['human_pick'] = unique_cards
        for bot_name in bot_names:
            accuracies = []
            for human_pick in unique_cards:
                all_picks = self.correct.loc[self.correct['human_pick']
                                             == human_pick]
                accuracies.append(
                    all_picks[bot_name].sum() / all_picks.shape[0])
