...
class DraftNetCustom(nn.Module):
    ...
    def __init__(self, set_tensor, use_features=False):
        ...
        """
        super(DraftNetCustom, self).__init__()
    ...
    def forward(self, x):
        ...
        if self.use_features:
            features = torch.mm(input, self.set_tensor_tranpose)
            input_and_features = torch.cat((input, features), 1)
            input_and_features = self.bn(input_and_features)
            input = input_and_features
