...
from draftsimtools import DraftNetCustom
...
# Makes label encoder
m19_set = pd.read_csv(
    data_folder + 'standardized_m19_rating.tsv', delimiter="\t")
...
def create_set_tensor(magic_set):
    ...
    reduced_set = magic_set[["Name", "Casting Cost 1",
                             "Card Type", "Rarity", "Color Vector"]]
    for index, row in reduced_set.iterrows():
        card_vector = create_set_vector(
            cmc_from_string(row[1]), row[2], row[3], row[4])
...
def train_net(net, dataloader, num_epoch, optimizer):
    ...
    for epoch in range(num_epoch):
        ...
        print('Train Cross-Entropy Loss: %.6f' %
...
split1_train_loader = torch.utils.data.DataLoader(
    split1_train, batch_size=128, shuffle=True)
split1_val_loader = torch.utils.data.DataLoader(
    split1_val, batch_size=128, shuffle=False)
split2_train_loader = torch.utils.data.DataLoader(
    split2_train, batch_size=128, shuffle=True)
split2_val_loader = torch.utils.data.DataLoader(
    split2_val, batch_size=128, shuffle=False)
split3_train_loader = torch.utils.data.DataLoader(
    split3_train, batch_size=128, shuffle=True)
split3_val_loader = torch.utils.data.DataLoader(
    split3_val, batch_size=128, shuffle=False)

# Creates networks
split1_net = DraftNetCustom(st, use_features=use_features)
split2_net = DraftNetCustom(st, use_features=use_features)
split3_net = DraftNetCustom(st, use_features=use_features)
final_net = DraftNetCustom(st, use_features=use_features)
if device.type != "cpu":
...
val_accuracies.to_csv('bots_data/val_accuracies_custom.csv', index=False)
...
for run in range(num_epochs):
    ...
    #torch.save(final_net, "draftnetCustom_jan31_2022_ep" + str(ep)  + ".pt")

# Writes final network and test accuracy to file
torch.save(final_net, "bots_data/draftnetCustom_jan31_2022_ep.pt")
with open('bots_data/test_accuracies_custom.txt', 'w') as f:
